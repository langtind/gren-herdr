...
# Also skip detached-HEAD worktrees (empty branch) — they'd show a blank line.
cands=$(printf '%s\n' "$wtjson" \
  | jq -r '.[] | select(.branch != null and .branch != "" and .is_main != true) | .branch' 2>/dev/null)
if [[ -z $cands ]]; then
...
# Guard an empty path: matching "" would target an unrelated workspace/pane.
...
# cwd is inside the removed checkout. (.cwd // "") guards panes with a null cwd —
# `null | startswith(...)` would otherwise abort the whole jq stream.
if [[ -n $wsid ]]; then
